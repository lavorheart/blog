...
(function() {
	...
	this.customLoginCallBack = function(loginStatus) {
		if (!loginStatus.result) {
			...
			window.location.replace("http://login.sina.com.cn/crossdomain2.php?action=login&r=" + encodeURIComponent(url));
		}
	};
	...
	var getReturnUrl = function() {
		...
		switch (sinaSSOController.entry) {
			...
			case "nphoto":
				...
				break;
			default:
				returnUrl = "http://login.sina.com.cn/";
		}
		return returnUrl;
	};
	//cookie 函数
	...
	var changePincode = function() {
		if( pined ) { 
			...
			pin.src = window.location.protocol + "//login.sina.com.cn/cgi/pin.php?r=" + random + "&s=0";
		}
	}
}).call(sinaSSOConfig);
